...
export default function Costs({ currentView, setCurrentView, user, isPremium, credits, handleLogout, setSelectedPlan }) {
  return (
      ...
      <main className="max-w-6xl mx-auto px-6 py-16 relative z-20 flex-1 w-full animate-fade-in">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 flex flex-col transform transition hover:-translate-y-2 hover:shadow-xl">
            ...
            <button onClick={() => { !user ? setCurrentView('register') : alert("Ya tienes este plan por defecto.") }} className="w-full py-3 rounded-xl font-bold text-[#6b2122] bg-rose-50 hover:bg-rose-100 transition border border-rose-200">Plan Actual</button>
          </div>
          ...
          <div className="bg-gradient-to-b from-[#6b2122] to-[#4a1516] rounded-2xl shadow-2xl border-2 border-amber-300 p-8 flex flex-col relative transform transition hover:-translate-y-2 hover:shadow-2xl scale-105 z-10">
            ...
            <button onClick={() => { setSelectedPlan({name: 'PyMes', price: 10}); setCurrentView('checkout'); }} className="w-full py-3 rounded-xl font-bold text-[#6b2122] bg-amber-300 hover:bg-amber-400 transition shadow-lg">Elegir Plan PyMes</button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 flex flex-col transform transition hover:-translate-y-2 hover:shadow-xl">
            ...
            <button onClick={() => { setSelectedPlan({name: 'Corporativo', price: 59}); setCurrentView('checkout'); }} className="w-full py-3 rounded-xl font-bold text-[#6b2122] bg-rose-50 hover:bg-rose-100 transition border border-rose-200">Elegir Plan Corporativo</button>
          </div>
        </div>
      </main>
